...
function ChatPage({ currentUser }) {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (data.lastThread && data.lastThread.threadId) {
          ...
          setPrevChats(normalizeMessages(data.lastThread.messages || []));
          setNewChat(false);
        } else {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          // Fallback: handle old "status": "success" format
            ...
            ws.close();
            resolve(response.rag_response || response.message || "Upload completed");
          }
          else {
            ws.close();
            resolve(response.message || "Upload completed");
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          // Fallback: handle old rag_response format
            ...
            ws.close();
            resolve(response.message || response.answer || "No response content");
          }
        } catch (e) {
          ws.close();
  ...
  const getReply = async () => {
    ...
    try {
      ...
      formData.append("message", fullContentString || "Processing file...");
      ...
      const finalReply = ragOutput || data.reply || "No response received";
        ...
        <div className="chat-messages-area">
          ...
          <div className="messages-list">
            {prevChats.map((chat, idx) => (
              <div
                className={chat.role === "assistant" ? "gpt-div" : "user-div"}
                key={idx}
              >
                {chat.role === "assistant" ? (
                    ...
                    {chat.attachments && chat.attachments.length > 0 && (
                      <div className="msg-attachments">
                        {chat.attachments.map((file, fIdx) => {
                          ...
                          return (
                            <div className="msg-file-card" key={fIdx}>
